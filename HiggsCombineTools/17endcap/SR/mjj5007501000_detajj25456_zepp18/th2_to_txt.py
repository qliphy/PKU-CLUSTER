...
jer=[1.02,1.001,1.005,1.059,1.010,1.002,1.006]
jes=[1.03,1.044,1.048,1.08,1.045,1.068,1.067]
pdf_QCD=[1.01,1.01,1.03,1.01,1.01,1.02,1.01]
pdf_ewk=[1.04,1.04,1.04,1.04,1.04,1.04,1.05]
scale_QCD=[1.15,1.21,1.18,1.09,1.10,1.10,1.09]
scale_ewk=[1.04,1.06,1.09,1.03,1.06,1.10,1.05]
fakephoton=[1.11,1.09,1.13,1.07,1.10,1.09,1.07];
...
for i in range(1,8):
   f = open('%s_%s_bin_%d.txt'%('17', 'endcap', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on 17_endcap\n'%('17','endcap'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on 17_endcap\n'%('17','endcap',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('17','endcap',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('17','endcap',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('17','endcap',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('17','endcap',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('17','endcap',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('17','endcap',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('17','endcap',i,WA_binerror))
