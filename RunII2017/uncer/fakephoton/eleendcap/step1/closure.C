...
#define num 6
using namespace std;
Double_t lowpt[ptnumber] ={20,25,30,35,60};//j
Double_t highpt[ptnumber]={25,30,35,60,400};

Double_t lpt[num]= {20,25,30,35,50,60};//i
Double_t hpt[num]={25,30,35,50,60,400};
...
TString dir = "/home/pku/anying/cms/RunII2017/MakeTemplate/pesudo/eleendcap/txt/";
TString dir1 = "/home/pku/anying/cms/RunII2017/MakeTemplate/pesudo/eleendcap/roofit/txt/";
...
//    cout<<fixed<<setprecision(0)<<ptlow<<"~"<<fixed<<setprecision(0)<<pthigh<<"\t"<<chisolow<<"~"<<chisohigh<<"\t"<<fixed<<setprecision(3)<<FakeNumber<<"\t"<<fitFake_number<<"\t"<<fabs(FakeNumber-fitFake_number)/FakeNumber<<endl;
 ...
 void run_calculate(Int_t a,Int_t b){
	...
	if(lowchiso[j]==5) low = lowchiso[j];
        if(highchiso[j]==10) high = highchiso[j];
     }
