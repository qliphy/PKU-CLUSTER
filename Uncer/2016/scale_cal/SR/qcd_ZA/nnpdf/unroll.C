...
void cmsLumi(bool channel) 
{
        ...
        latex.DrawLatex(0.76,0.92,Form("35.8 fb^{-1} (%d TeV)", (beamcomenergytev)));
}
...
//    TH1D* h1_out= new TH1D(hname, "", nbins-1, 0, nbins-1);
	for(Int_t iy=1; iy<=nbinsy; iy++){
		for(Int_t ix=1; ix<=nbinsx; ix++){
			...
			cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<35.8*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
		}
	}
        for(Int_t i=0;i<6;i++){
...
//	Double_t mjj_bins[4]={500, 800, 1200, 2000};
	...
	const char *name[9]={"Mjj 500~2000"};
...
//	const char *name[6]={"Mjj 500~800","Mjj 800~1200","Mjj 500~800","Mjj 800~1200","Mjj 500~800","Mjj 800~1200"};
    ...
    for(Int_t i=0;i<num;i++){
        ...
        t_ZA[i]= unroll(th2_ZA[i], mjj_bins, detajj_bins, 1,1,Form("hist_%d",i+1));
...
//        file2<<t_ZA[i]->GetBinContent(k+1)*scale_factor[i]<<endl;}
     ...
     t_ZA[0]->GetYaxis()->SetRangeUser(-2,60);
//     t_ZA[0]->Draw("HIST");
