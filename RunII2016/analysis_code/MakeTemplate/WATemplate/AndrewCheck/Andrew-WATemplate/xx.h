...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 bool Photon_cut[6],LEP;
 Double_t lowpt[ptnumber]= {25,30,40,50,70,100,135};
 Double_t highpt[ptnumber]={30,40,50,70,100,135,400};
 Int_t    bin=20;
 Double_t xlow=0;
 Double_t xhigh=0.0202;
 Double_t Sieie_wa;
   TFile* f1;
   TFile* fmcwa[ptnumber];
   TFile* fdata[ptnumber];
   TH1F* h_mcwa[ptnumber];
   TH1F* h_datawa[ptnumber];
   TH1F* h_sieie[ptnumber];
   TH1F* h_waFc[ptnumber];
   TH1F* h_mcFc[ptnumber];
   TH1F* h_mcwaT[ptnumber];
   TH1F* h_waFs[ptnumber];
   TTree* tree_mcwa[ptnumber];
   TTree* tree_datawa[ptnumber];
...
void xx::endJob(TString name) {
     ...
     if(name.Contains("SMu")){
       for(Int_t i=0;i<ptnumber;i++){fdata[i]->cd();tree_datawa[i]->Write();fdata[i]->Close();delete fdata[i]; }
       }
     if(name.Contains("WA")){
       for(Int_t i=0;i<ptnumber;i++){fmcwa[i]->cd();tree_mcwa[i]->Write();fmcwa[i]->Close(); delete fmcwa[i];}
       }
  }
