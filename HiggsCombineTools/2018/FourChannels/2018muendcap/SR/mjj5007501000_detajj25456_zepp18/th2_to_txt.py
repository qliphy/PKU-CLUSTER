...
jer=[1.008,1.007,1.006,1.041,1.012,1.013,1.077,1.006]
jes=[1.033,1.015,1.063,1.026,1.050,1.033,1.050,1.050]
...
fakephoton=[1.261,1.225,1.297,1.244,1.265,1.253,1.253,1.258];
QCDZA_scale_extra_down = [0.97,0.954,0.922,1.008,0.94,0.966,1.014,0.966];
QCDZA_scale_extra_up = [1.035,1.057,1.094,0.984,1.061,1.038,0.977,1.025];
for i in range(1,9):
   f = open('%s_%s_bin_%d.txt'%('muon18', 'endcap', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on muon18_endcap\n'%('muon18','endcap'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on muon18_endcap\n'%('muon18','endcap',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('muon18','endcap',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('muon18','endcap',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('muon18','endcap',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('muon18','endcap',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('muon18','endcap',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('muon18','endcap',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('muon18','endcap',i,WA_binerror))
