...
public:
	...
	EDBRHistoPlotter(std::string nameInDir,
			...
			std::vector<std::string> nameFileMCSig,std::string nameFileContamination,double targetLumi,
		...
		nameFileContamination_ = nameFileContamination;
	...
	std::string nameFileContamination_;
...
//  TPad* fPads3 = NULL;
	...
	for (size_t i = 0; i != fileNamesMC.size(); ++i) {
        ...
        cout<<"fileNamesMC "<<fileNamesMC.at(i).c_str()<<endl;
	}
        filesMC.push_back(TFile::Open((nameInDir_ + "cutla-out"+ nameFileContamination_ + ".root"))); 

/*	double sumBkgOther = 0.;
*/      cout<<"filesMC.size() "<<filesMC.size()<<endl;
	for (size_t i = 0; i != filesMC.size(); ++i) {
	...
	cout<<"histosMC.size() "<<histosMC.size()<<endl;
	if (histosMC.size() != 0) {
	...
	// Make a histogram just for the sum
        int plj_index,contamination_index;
	for (size_t i = 0; i != histosMC.size(); ++i) {
		histosMC.at(i)->SetFillColor(getFillColor(i));
		TString filename = filesMC.at(i)->GetName();
                if(filename.Contains("plj")) plj_index=i;
                if(filename.Contains("Contamination")){
			contamination_index=i;
			sumMC->Add(histosMC.at(i),-targetLumi_);
                        TH1D* htmp = NULL; htmp->Add(histosMC.at(i),-targetLumi_); 
                        hs->Add(htmp);
		}
		else{
...
//	histosMC.at(plj_index)->Add(histosMC.at(contamination_index),-targetLumi_);
//        sumMC->Add(histosMC.at(plj_index));
//        hs->Add(histosMC.at(plj_index));
