...
TFile* fdata = TFile::Open("/home/pku/anying/cms/RunII2016/analysis_code/MakeTemplate/ZAJetfakephoton/MuonBarrel/root/Data_template-cutlep-outDMuon16.root");
TFile* ftrue = TFile::Open("/home/pku/anying/cms/RunII2016/analysis_code/MakeTemplate/ZAJetfakephoton/MuonBarrel/root/True_template-cutlep-outZA.root");
TFile* ffake = TFile::Open("/home/pku/anying/cms/RunII2016/analysis_code/MakeTemplate/ZAJetfakephoton/MuonBarrel/root/Fake_template-cutlep-outDMuon16.root");
...
//        fit->Constrain(1,0,1);
        ...
        if (status == 0) {                       // check on fit status
	        ...
	        double fit_err = mcp0->Integral(0, mcp0->FindFixBin(0.0102)-1)*result->Integral()*errP0/mcp0->Integral();
            fr = p0* (hfake->Integral(0,hfake->FindFixBin(0.0102)-1)/hfake->Integral())/(result->Integral(0,result->FindFixBin(0.0102)-1)/result->Integral());

            fakeValue = mcp0->IntegralAndError(0,nBins,fakeError); 
            fake_window = mcp0->IntegralAndError(0,mcp0->FindFixBin(0.0102)-1,fake_window_Error);
	        real_window = mcp1->IntegralAndError(0,mcp1->FindFixBin(0.0102)-1,real_window_Error);
            data_window = result->IntegralAndError(0,result->FindFixBin(0.0102)-1,data_window_Error);
...
void run_fitf(Int_t b){
  float lowchiso[21];
  float highchiso[21];
 Double_t lowpt[ptnumber] ={20,25,30,35,40,50,65,100,125};
 Double_t highpt[ptnumber]={25,30,35,40,50,65,100,125,400};
