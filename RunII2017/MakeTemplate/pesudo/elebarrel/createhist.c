...
Double_t s[6];
 ...
 Int_t nlooseeles,nloosemus,lep;
...
for(Int_t i=2;i<6;i++){
 ...
 l1->AddEntry(h2[1],"photon_chiso<1.141"); 
...
// nentries=100;
 ...
 for(Int_t i=0;i<nentries;i++){
    t1->GetEntry(i);
    for(Int_t j=0;j<6;j++){
         Zjets[j] = lep ==11 && ptlep1 > 25. && ptlep2 > 25. && abs(etalep1) < 2.5 &&abs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0 && massVlep >70. && massVlep < 110;
         Leading_photon[j]=photon_isprompt[j] != 1  &&  photon_drla[j]>0.7 && photon_drla2[j]>0.7 && photon_drla[j]<10 && photon_drla2[j]<10;// && photon_pt[j]<35.&&photon_pt[j]>25;
