...
//	TFile*fin = TFile::Open("../hgg/transformation_barrel.root");
	TFile*fin = TFile::Open("../hgg/transformation_endcap.root");
//	TGraph* gbarrel;
	TGraph* gendcap;
//	fin->GetObject("barrel",gbarrel);
	fin->GetObject("endcap",gendcap);
	TFile* fdata=TFile::Open("../data_MC/smallDEle.root");
	TTree*tdata=(TTree*)fdata->Get("DEle");
	TFile* fmc=TFile::Open("../data_MC/smallZJets_FX.root");
	TTree*tmc=(TTree*)fmc->Get("ZJets_FX");
//	TH1D* hdata = trans(tdata,"DEle",gbarrel);
//	TH1D* hmc = trans(tmc,"ZJets",gbarrel);
	TH1D* hdata = trans(tdata,"DEle",gendcap);
