...
het[0] = new TH1D("hclusEt_ele","clusEt",20,0,200);
heta[0] = new TH1D("hdetEta_ele","detEta",6,etabins);
...
het[1] = new TH1D("het_eghlt","et",6,etbins);
heta[1] = new TH1D("heta_eghlt","eta",6,etabins);
...
//het[1] = new TH1D("het_eghltUS","et",6,etbins);
//heta[1] = new TH1D("heta_eghltUS","eta",6,etabins);
//hnrgy[1] = new TH1D("hnrgy_eghltUS","nrgy",40,0,400);
//hphi[1] = new TH1D("hphi_eghltUS","phi",13,-6.5,6.5);
//hsieie[1] = new TH1D("hsieie_eghltUS","sigamIEtaIEta",40,0,0.06);
//detain[1] = new TH1D("hetain_eghltUS","dEtaIn",40,0,0.02);
//dphiin[1] = new TH1D("hphiin_eghltUS","dPhiIn",40,0,0.05);
//hMhits[1]= new TH1D("hMHits_eghltUS","missing hits",7,0,6);
...
//     if(jentry%1000==0&&egHLT_et>0){ cout<<jentry<<";"<<egHLT_et<<"; "<<egHLT_eta<<";"<<egHLT_nrgy<<endl;}
      if(clusEt>200) clusEt =201; if(egHLT_et>200) egHLT_et =201;
//      if(!(eleTruthEt>20)) continue;
...
//      het[0]->Fill(clusEt);het[1]->Fill(egHLTUS_et);hnrgy[0]->Fill(clusNrgy);hnrgy[1]->Fill(egHLTUS_nrgy);
//      heta[0] ->Fill(detEta);heta[1] ->Fill(egHLTUS_eta);	hphi[0]->Fill(detPhi);hphi[1]->Fill(egHLTUS_phi);
//      hsieie[0]->Fill(sigmaIEtaIEta);hsieie[1]->Fill(egHLTUS_sigmaIEtaIEta);
//      detain[0]->Fill(dEtaInSeed);detain[1]->Fill(egHLTUS_dEtaIn);
//      dphiin[0]->Fill(dPhiIn);dphiin[1]->Fill(egHLTUS_dPhiIn);
//      hMhits[0]->Fill(nrMissHits);hMhits[1]->Fill(egHLTUS_nrMissHits);
