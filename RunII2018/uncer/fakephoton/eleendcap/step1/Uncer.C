...
TFile* f1 = new TFile("/home/pku/anying/cms/RunII2018/plj/plj-plot/EleEndcap/plj-hist/cutla-outDEle_plj_he.root");
TFile* f2 = new TFile("/home/pku/anying/cms/RunII2018/plj/plj-plot/EleEndcap/plj-hist/cutla-outDEle_he.root");
TFile* f3 = new TFile("/home/pku/anying/cms/RunII2018/plj/plj-plot/EleEndcap/plj-hist/cutla-outZA_contamination_he.root");
...
for(Int_t i=0;i<num;i++){
	 if(j>=ptnumber-1) j=ptnumber-1;
          ...
          else {j = j+1; run_calculate(j,i);}
          j++;
	 if(j>=ptnumber-1) j=ptnumber-1;

	 open(lpt[i],hpt[i],"ZA1","ZA2","ZA");
	 open(lpt[i],hpt[i],"EWK","ZA");
	 f4<<fixed<<setprecision(0)<<lpt[i]<<"~"<<hpt[i]<<"\t"<<fixed<<setprecision(3)<<closure_uncer<<"\t\t"<<fixed<<setprecision(2)<<sideband_uncer<<"\t\t"<<truetemplate_uncer<<"\t\t"<<sqrt(closure_uncer*closure_uncer+sideband_uncer*sideband_uncer+truetemplate_uncer*truetemplate_uncer)<<endl;
