...
//	Double_t mjj_bins[4]={500, 750, 1000, 2000};
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
        p=0;
		t->GetEntry(j);
        if( !(lep==13 && fabs(photoneta)<1.4442 && Mjj>500 && detajj>2.5) ) continue;
        for(Int_t i=0;i<9;i++){
...
int d_hist(){
	run("/home/pku/anying/cms/RunII2017/aa2/MuonBarrel/output-slimmed-rootfiles/optimal_ZA_pweight");
