...
#include <vector.h>
	...
	TH1D* h31= (TH1D*) ZG_optimal_outphotonlikejet_mu_mediumlikephoton_up->Clone();
	h31->Sumw2();
	TH1D* h41= (TH1D*) ZG_optimal_bkg_up->Clone();
	h41->Sumw2();
	h41->Add(h31);
	h41->SetName("background_mu_backshapeUp");

	TH1D* h32= (TH1D*) ZG_optimal_outphotonlikejet_mu_mediumlikephoton_down->Clone();
	h32->Sumw2();
	TH1D* h42= (TH1D*) ZG_optimal_bkg_down->Clone();
	h42->Sumw2();
	h42->Add(h32);
	h42->SetName("background_mu_backshapeDown");

	TFile *fout = new TFile("mu_Limit_SM.root", "RECREATE");
	fout->cd(); 
	h1->Write();   
	h2->Write();   
	h4->Write();   
