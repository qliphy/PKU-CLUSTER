...
jer=[1.001,1.0002,1.007,1.010,1.003,1.006,1.019,1.017]
jes=[1.012,1.004,1.054,1.044,1.007,1.061,1.097,1.067]
pdf_QCD=[1.01,1.01,1.03,1.01,1.01,1.02,1.01,1.01]
pdf_ewk=[1.04,1.04,1.04,1.04,1.04,1.04,1.05,1.04]
scale_QCD=[1.15,1.16,1.20,1.10,1.12,1.14,1.11,1.08]
scale_ewk=[1.04,1.06,1.09,1.03,1.06,1.11,1.05,1.13]
fakephoton=[1.144,1.167,0,1.114,1.138,1.222,1.116,1.129];
QCDZA_scale_extra_down = [0.97,0.954,0.922,1.008,0.94,0.966,1.014,0.966];
QCDZA_scale_extra_up = [1.035,1.057,1.94,0.984,1.061,1.038,0.977,1.025];
for i in range(1,9):
   f = open('%s_%s_bin_%d.txt'%('ele18', 'barrel', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on ele18_barrel\n'%('ele18','barrel'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on ele18_barrel\n'%('ele18','barrel',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('ele18','barrel',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('ele18','barrel',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('ele18','barrel',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('ele18','barrel',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('ele18','barrel',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('ele18','barrel',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('ele18','barrel',i,WA_binerror))
   f.write('JES_18\tlnN\t%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(jes[i-1],jes[i-1],jes[i-1],jes[i-1],jes[i-1],jes[i-1]))
   f.write('JER_18\tlnN\t%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(jer[i-1],jer[i-1],jer[i-1],jer[i-1],jer[i-1],jer[i-1]))
   ...
   f.write('%s_efficiency\tlnN\t1.02\t1.02\t-\t1.02\t1.02\t1.02\t1.02\n'%('ele18'))
#   f.write('pileup\tlnN\t1.01\t1.01\t-\t1.01\t1.01\t1.01\t1.01\n')
   f.write('photon_id_18\tlnN\t1.03\t-\t-\t1.03\t1.03\t1.03\t1.03\n')
