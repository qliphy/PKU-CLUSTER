...
void EDBRHistoMaker::createAllHistos() {
	...
	for (int i = 0; i != nVars; ++i) {
		sprintf(buffer, "%s_elebarrel", hs.vars[i].c_str());
...
//data
		if ( lep == 11 &&  (HLT_Ele1>0 || HLT_Ele2 > 0) && ptlep1 > 25. && ptlep2 > 25. && fabs(etalep1) < 2.5 && fabs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0 && massVlep > 70. && massVlep < 110. && photonet > 20.&& fabs(photoneta) < 1.4442 && drla>0.7&&drla<10&&drla2>0.7&&drla2<10  && jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7&&  !(fabs(jet2eta)<3&&fabs(jet2eta)>2.7) && !(fabs(jet1eta)<3&&fabs(jet1eta)>2.7) && Mjj > 150&&Mjj<400&&ZGmass>100) {
			//if(Mjj<400)
			numbe_out++;
			//cout<<"L1 = "<<L1<<endl;
			treename->Fill();
