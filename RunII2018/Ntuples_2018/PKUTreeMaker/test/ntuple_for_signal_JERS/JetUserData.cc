...
void JetUserData::produce( edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	for (size_t i = 0; i< jetColl->size(); i++){
		...
		std::cout<<jet.pt()<<" "<<rawJetP4.eta()<<" "<<rawJetP4.pt()<<" "<<jetCorrFactor*rawJetP4.pt()<<" "<<smearedP4.Pt()<<" "<<jecUncertainty_up<<" "<<" "<<jecUncertainty_down<<" "<<jecUncertainty_l1_up<<" "<<jecUncertainty_l1_down<<" "<<jetCorrFactor<<" "<<jetCorrFactor_l1<<std::endl;
