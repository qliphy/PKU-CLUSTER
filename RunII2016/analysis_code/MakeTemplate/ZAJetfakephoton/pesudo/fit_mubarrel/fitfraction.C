...
//        fit->Constrain(1,0,1);
	    ...
	    double fit_err = mcp0->Integral(0, mcp0->FindFixBin(0.01022))*result->Integral()*errP0/mcp0->Integral();
            fr = p0* (hfake->Integral(0,hfake->FindFixBin(0.01022))/hfake->Integral())/(result->Integral(0,result->FindFixBin(0.01022))/result->Integral());

            fakeValue = mcp0->IntegralAndError(0,nBins,fakeError); 
            fake_window = mcp0->IntegralAndError(0,mcp0->FindFixBin(0.01022),fake_window_Error);
            data_window = result->IntegralAndError(0,result->FindFixBin(0.01022),data_window_Error);
	    ...
	    mcp1->Scale(result->Integral()*(1-p0)/mcp1->Integral());//scale mc prediction
            ...
            mcFake = hist->IntegralAndError(0,hist->FindFixBin(0.01022),mcFake_err);
...
//            fr = p0* (hfake->Integral(1,hfake->GetXaxis()->FindFixBin(0.01022)-1)/hfake->Integral())/(hdata->Integral(1,hdata->GetXaxis()->FindFixBin(0.01022)-1)/hdata->Integral());
//            fr = p0* (hfake->Integral(1,hfake->FindFixBin(0.01022))/hfake->Integral())/(hdata->Integral(1,hdata->FindFixBin(0.01022))/hdata->Integral());
	    ...
	    ofstream file1(dir + TString("FakeNumber_") + Form("pt%0.f-%0.f_chiso%0.f-%0.f.txt", lowpt, highpt,lowchiso,highchiso),ios::out);
            ...
            myfile <<fr<<"\t"<<fr_Error<<"\t"<<hfake->Integral(0,hfake->FindFixBin(0.01022))<<"\t"<<fakeValue<<"\t"<<fake_window<<endl;
