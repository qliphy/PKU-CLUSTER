	...
	for(Long64_t i=0;i<entries;i++){
		...
		double pt_tmp=-99;
                ...
                for(int j=0;j<6;j++){
                        ...
                        if(photon_pt[j]>15 && photon_hoe[j]<0.0326 && photon_nhiso[j]<(2.718 + 0.0117*photon_pt[j] + 0.000023*photon_pt[j]*photon_pt[j]) && photon_phoiso[j]<(3.867 + 0.0037*photon_pt[j]) && photon_chiso[j]<1.051 && tm1>70 &&tm1<110) {
...
int main(){
	...
	TFile* fdata=TFile::Open("./samples/endcap/smallDEle16.root");
	TTree*tdata=(TTree*)fdata->Get("DEle16");
	TFile* fmc=TFile::Open("./samples/endcap/smallZJets_FX.root");
