...
void calculate(Double_t lowpt,Double_t highpt,Double_t lowchiso,Double_t highchiso){
    ...
    f2.open(Form(dir + "TrueNumber_pt%0.f-%0.f_chiso%0.f-%0.f.txt",lowpt,highpt,lowchiso,highchiso));
    ...
    f4.open(Form(dir + "FakeNumber_pt%0.f-%0.f_chiso%0.f-%0.f.txt",lowpt,highpt,lowchiso,highchiso));
...
//  myfile<<"*"<<"\t"<<"3"<<"\t"<<"4"<<"\t"<<"5"<<"\t"<<"6"<<"\t"<<"7"<<"\t"<<"8"<<endl;
    ofstream myfile2(Form("closure-test_pt%0.f-%0.f.txt",lowpt[i],highpt[i]));
      for(Int_t j=0;j<21;j++){
         run_calculate(i,j);
