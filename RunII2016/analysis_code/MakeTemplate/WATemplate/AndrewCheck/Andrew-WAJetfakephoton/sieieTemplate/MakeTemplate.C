...
//   nentries = 1000;
	    ...
	    { 
         ...
         if(name.Contains("outDMuon")==1){
	       ...
	       photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k])
		      {h3[k]->Fill(photon_sieie[position]);df++;}//data fake ISR
		   ...
		   photon_pt[position]>pttruelow[k] && photon_pt[position]<pttruehigh[k]*/)
	               {h4[k]->Fill(photon_sieie[position]);dt++;}//data true FSR

	     if(photon_drla[position]>0.4 && photon_drla2[position]>0.4 &&
	        TMath::Min(photon_drla[position],photon_drla2[position])<1&&
                photon_sieie[position]<0.01022 && massVlep<80 &&
		Mva>81 && Mva<101 &&
	        photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k])
	               {hsieie[k]->Fill(photon_sieie[position]);}//data true photon*/
	 }
      }
         vector_pt.clear();
   }
  ...
  if(name.Contains("outDMuon")==1)  cout<<"data true "<<dt<<endl;
   ...
   if(name.Contains("outZA")){
       f1 = new TFile("./mcFake_template_check.root","recreate");
       f1->cd();
   ...
   if(name.Contains("outDMuon")){
       f3 = new TFile("./dataFake_template_check.root","recreate");
       f4 = new TFile("./dataTrue_template_check.root","recreate");
   ...
   if(name.Contains("outZJets")){
       f2 = new TFile("./mcTrue_template_check.root","recreate");
       f2->cd();
