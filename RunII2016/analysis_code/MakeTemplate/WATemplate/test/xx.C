...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for(Int_t i=0;i<ptnumber;i++){
      ...
      if(name.Contains("SMu")){
           ...
           h_waFs[i]=new TH1F(Form("hwaFs_pt%0.f-%0.f",lowpt[i],highpt[i]),"data template",bin,0.,0.03);
        ...
        if( photon_drla[position]>0.4){
            ...
            if(name.Contains("SMu") && 
               photon_pt[position]>lowpt[k]&&photon_pt[position]<highpt[k])
               {
                   ...
                   if(photon_sieie[position]>0.012&&photon_sieie[position]<0.02){
                      h_waFs[k]->Fill(photon_sieie[position]);}
               }
        }//end if
      }//end for loop
      vector_pt.clear();
...
if(name.Contains("SMu")){
   for(Int_t i=0;i<ptnumber;i++){f1->cd();h_datawa[i]->Write();h_sieie[i]->Write();h_waFs[i]->Write();}
 }
input1->Close();
