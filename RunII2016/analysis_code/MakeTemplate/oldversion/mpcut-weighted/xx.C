   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for(Int_t k=0;k<9;k++){
        if( photon_drla[position]>0.4 &&
            ...
            ExTree->Fill();
            if(name.Contains("WA")){h_mcwa[k]->Fill(photon_sieie[position],scalef*lumiWeight*pileupWeight);}
...
if(name.Contains("WA")){
   for(Int_t i=0;i<9;i++){f1->cd();h_mcwa[i]->Write();}
 }
if(name.Contains("SMu")){
   for(Int_t i=0;i<9;i++){f1->cd();h_datawa[i]->Write();}
 }
input1->Close();
