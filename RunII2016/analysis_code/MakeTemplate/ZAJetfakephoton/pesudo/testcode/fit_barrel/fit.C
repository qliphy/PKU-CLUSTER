...
TString dir = "../barrel/roofit/txt/";
TString dir1 = "../barrel/root/";
ofstream file3("../barrel/roofit/info_roofit.txt");
void fit(float lowpt, float highpt, float lowchiso, float highchiso){
//TString b="chiso5-12_";
	TFile* fdata = TFile::Open(Form(dir1 + "datap_lep_fake_pt%0.f-%0.f_chiso%0.f-%0.f.root",lowpt,highpt,lowchiso,highchiso));
	TFile* ftrue = TFile::Open(Form(dir1 + "ZA-EWK_photonreal_pt%0.f_%0.f.root",lowpt,highpt));
...
//	TFile* ffake = TFile::Open(Form("../fit_produce/ZJets_FX_%0.f_%0.f_fake.root", lowpt, highpt));
	...
	TH1F* hist_ = (TH1F*)fdata->Get(Form("hfake_pt%0.f-%0.f",lowpt,highpt));
	TH1F* hist1_ = (TH1F*)fdata->Get(Form("htrue_pt%0.f-%0.f",lowpt,highpt));
