...
//    TH1D* h1_out= new TH1D(hname, "", nbins-1, 0, nbins-1);
    TH1D* h1_out= new TH1D(hname, "", 1, 0, 1);
...
//		    cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<41.52*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
    ...
    h1_out->SetBinContent(1,h1->GetBinContent(1) +h1->GetBinContent(2) +h1->GetBinContent(3) +h1->GetBinContent(4) +h1->GetBinContent(5) +h1->GetBinContent(6) +h1->GetBinContent(7) + h1->GetBinContent(8) + h1->GetBinContent(9) );
/*    for(Int_t i=0;i<6;i++){
