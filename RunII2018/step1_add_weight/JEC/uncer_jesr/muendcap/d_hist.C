...
//	t->SetBranchAddress("muon_hlt_scale",&muon_hlt_scale);
	...
	{
		t->GetEntry(i);
                if(  !(lep==13 &&  (fabs(photoneta)<2.5&&fabs(photoneta)>1.566)) )
                    continue;
                Mjj_JEC[0] = Mjj_new;
...
//                actualWeight = scalef * pileupWeight*lumiWeight * muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*muon1_track_scale*muon2_track_scale*muon_hlt_scale;
		...
		for(int i=0;i<3;i++){
			if(Mjj_JEC[0]<2000 && deltaeta<6.5)  th2[i]->Fill(Mjj_JEC[i], deltaeta, actualWeight);
			if(Mjj>=2000 && deltaeta<6.5) th2[i]->Fill(1999, deltaeta, actualWeight);
			if(Mjj<2000 && deltaeta>=6.5) th2[i]->Fill(Mjj_JEC[i], 6.1, actualWeight);
			if(Mjj>=2000 && deltaeta>=6.5)th2[i]->Fill(1999, 6.1, actualWeight);
		}
	}
}
