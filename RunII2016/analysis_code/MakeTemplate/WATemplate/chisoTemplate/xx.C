...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for(Int_t i=0;i<ptnumber;i++){
	  ...
	  tree_mcwa[i]->Branch("chiso",&chiso_wa);}
      if(name.Contains("SMu")){
           h_datawa[i]=new TH1F(Form("hdatawa_pt%0.f-%0.f",lowpt[i],highpt[i]),"data template",bin,0,xhigh);
           h_sieie[i]=new TH1F(Form("hsieie_pt%0.f-%0.f",lowpt[i],highpt[i]),"qualified photon",bin,0,xhigh);
           h_waFc[i]=new TH1F(Form("hwaFc_pt%0.f-%0.f",lowpt[i],highpt[i]),"invert chiso and fill sieie",bin,0,xhigh);
           h_waFs[i]=new TH1F(Form("hwaFs_pt%0.f-%0.f",lowpt[i],highpt[i]),"invert sieie and fill chiso",bin,0,xhigh);
	  ...
	  tree_datawa[i]->Branch("chiso",&chiso_wa);}
   }
   //nentries=100;
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     //AddWeight(); 
      ...
      chiso_wa=photon_chiso[position];
      if(jentry%10000==0){
      ...
      for(Int_t k=0;k<ptnumber;k++){
        if( photon_drla[position]>0.4){
		   ...
		   if(photon_sieie[position]<0.01022){
			   h_mcwa[k]->Fill(photon_chiso[position],actualWeight);
			   fmcwa[k]->cd();
			   tree_mcwa[k]->Fill(); 
		   }//save the qualified variable in a tree 
	   if( photon_sieie[position]>0.012 && photon_sieie[position]<0.2 && photon_isprompt[position]==2 ){
                      h_mcFc[k]->Fill(photon_chiso[position],actualWeight);
		   }//the contamination of true photons after inverting in the fake template
		   if( photon_sieie[position]<0.01022 && photon_isprompt[position]==2 ){
                      h_mcwaT[k]->Fill(photon_chiso[position],actualWeight);
		   }//building the true template from WA mc
	       }
            if(name.Contains("SMu") && 
               photon_pt[position]>lowpt[k]&&photon_pt[position]<highpt[k])
               {
		   if(photon_sieie[position]<0.01022)
		   {       h_datawa[k]->Fill(photon_chiso[position]);
			   fdata[k]->cd();
			   tree_datawa[k]->Fill(); 
		   }//data template
                   if(photon_chiso[position]<0.441 && photon_sieie[position]<0.01022){
                      h_sieie[k]->Fill(photon_sieie[position]);}//the number of real photon
                   if(photon_chiso[position]<10&&photon_chiso[position]>4){
                      h_waFc[k]->Fill(photon_sieie[position]);}//build fake template by inverting chiso
                   if(photon_sieie[position]>0.012&&photon_sieie[position]<0.2){
                      h_waFs[k]->Fill(photon_chiso[position]);}//build fake template by inverting sieie
               }
        }//end if
      }//end for loop
      vector_pt.clear();
