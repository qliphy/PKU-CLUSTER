...
// set mcsig information
	...
	for (int i = 0; i < nDATA; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018_%s.root", dataLabels[i].c_str());
...
//		std::cout<<"OK"<<std::endl;
	...
	for (int i = 0; i < nMC; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018_%s.root", mcLabels[i].c_str());
	...
	for (int  i = 0; i < nMCSig; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018_%s.root", mcLabelsSig[i].c_str());
