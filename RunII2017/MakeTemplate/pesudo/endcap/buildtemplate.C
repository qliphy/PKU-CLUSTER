...
TString sieiecut = "nosieiecut_endcap";
...
double delta_R(Double_t eta1, Double_t phi1, Double_t eta2, Double_t phi2);
 ...
 bool LEPmu,LEPele,Leading_photon[6],medium_cut[6],loose_cut[6],JET[6],jet_flag;
...
// nentries=100000;
 ...
 for(Int_t i=0;i<nentries;i++){
    tree->GetEntry(i);
    LEPmu = lep==13&& (HLT_Mu1>0 || HLT_Mu2>0) && ptlep1 > 20. && ptlep2 > 20.&& abs(etalep1) < 2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep >70. && massVlep < 110 && ngoodmus>1;
    LEPele = lep==11&& (HLT_Ele1>0 || HLT_Ele2>0) && ptlep1 > 25. && ptlep2 > 25.&& abs(etalep1) < 2.5 &&abs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0 && massVlep >70. && massVlep < 110 ;
    jet=0;
	...
	//Muon Endcap		 
	 ...
	 if(( LEPmu || LEPele) && Leading_photon[j]&&medium_cut[j] /*&& (jet_flag==true)*/ ){
    ...
    if(fabs(scalef)<0.0029&&fabs(scalef)>0.0028)  za=true;
...
void histo(){
 ...
 Double_t xhigh= 0.0572;
  for(Int_t i=0;i<ptnumber;i++){
      ...
      h13[i]=new TH1D(Form("h13_pt%0.f_%0.f",lowpt[i],highpt[i]),"chiso<1.051",bin,0,xhigh);////Zjets && leding_photon && medium_cut && photon_chiso>2
    for(Int_t j=0;j<21;j++){  
