	 ...
	 ,effAreaPhotons_((iConfig.getParameter<edm::FileInPath>("effAreaPhoFile")).fullPath() )
{
	...
	photonSrc_      = consumes<edm::View<pat::Photon>>(iConfig.getParameter<edm::InputTag>( "photons") ) ;
...
//	std::cout<<"matchedgenMu2_pt "<<matchedgenMu2_pt<<std::endl;
	...
	for (size_t ip=0; ip<photons->size();ip++)
	{
		...
		double chiso=std::max(0.0, chIso1 - rhoVal_*EAch(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaChHadrons_.getEffectiveArea(fabs(phosc_eta)));
		//            double chiso=std::max((*photons)[ip].chargedHadronIso()-rhoVal_*EAch(fabs((*photons)[ip].eta())),0.0);
		double nhiso=std::max(0.0, nhIso1 - rhoVal_*EAnh(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaNeuHadrons_.getEffectiveArea(fabs(phosc_eta)));
		//            double nhiso=std::max((*photons)[ip].neutralHadronIso()-rhoVal_*EAnh(fabs((*photons)[ip].eta())),0.0);
		double phoiso=std::max(0.0, phIso1 - rhoVal_*EApho(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaPhotons_.getEffectiveArea(fabs(phosc_eta)));
