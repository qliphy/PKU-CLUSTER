...
//   nentries = 100000;
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      jet=0;
      for(Int_t j=0;j<6;j++){
          Photon_cut[j]= /*photon_pev[j]==1*/ photon_pevnew[j]==1 && photon_hoe[j]<0.0396  && photon_nhiso[j]<2.725 + 0.0148*photon_pt[j] + 0.000017*photon_pt[j]*photon_pt[j] && photon_phoiso[j]<2.571 + 0.0047*photon_pt[j]&&abs(photon_eta[j])<1.4442&&photon_pt[j]>20&&photon_pt[j]<400&&photon_drla[j]>0.7&&photon_drla[j]!=10; 
...
//		      cout<<"fake contribution from ZA"<<endl;
   ...
   for(Int_t i=0;i<num;i++){
       ...
       cout<<Form(" %0.f<photon_pt<%0.f",lowpt[i],highpt[i])<<"  Data template: "<<"isprompt==1,TrueNumber = "<<TrueNumber[i]<<endl;
       cout<<Form(" %0.f<photon_pt<%0.f",lowpt[i],highpt[i])<<"  Data template: "<<"isprompt!=1,FakeNumber = "<<FakeNumber[i]<<endl;
       if(name.Contains("A")==1){
