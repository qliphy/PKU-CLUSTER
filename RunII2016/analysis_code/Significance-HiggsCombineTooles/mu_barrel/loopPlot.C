...
void loopPlot() {
	...
	std::string pathToTrees = "/data/pku/home/anying/cms/CMSSW_8_0_26_patch2/analysis_code/aa2/control-region/";
...
// Setup names of data files for trees.
	...
	std::cout << "set data imformation, we have " << nDATA << "data file"
			<< std::endl;
	std::string dataLabels[nDATA] = { "outDMuon" };
...
// set mc imformation
		...
		std::cout << "set data imformation, we have " << nMC << "mc file"
				<< std::endl;
		std::string mcLabels[nMC] = { "ST", "TTA", "VV", "WA","plj_weightb", "ZA" };
...
// set mcsig information
	...
	std::cout << "set data imformation, we have " << nMCSig << "mcsig file"
			<< std::endl;
	std::string mcLabelsSig[nMCSig] = { "outZA-EWK" };
	double kFactorsSig_array[nMCSig] = { 1 };
	std::vector < std::string > fMCSig;
	for (int ii = 0; ii < nMCSig; ii++) {
		fMCSig.push_back(pathToTrees+"optimal_cutla-" + mcLabelsSig[ii] + ".root");
	}
