...
// Tue Sep 11 03:34:26 2018 by ROOT version 6.06/01
// from TTree demo/demo
// found on file: cutLEP-outDMuon.root
//////////////////////////////////////////////////////////
...
#ifdef plj_cxx
plj::plj(TTree *tree,TString dataset) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("cutLEP-outDMuon.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("cutLEP-outDMuon.root");
      }
...
Int_t plj::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t plj::LoadTree(Long64_t entry)
{
