 ...
 for(Int_t i=0;i<nentries;i++){
    ...
    for(Int_t j=0;j<6;j++){
         ...
         medium_cut[j]= /*photon_pevnew[j]==1 &&*/
                 ...
                 photon_pt[j]>20&&photon_pt[j]<400;//muon endcap 
    ...
    for(Int_t k=0;k<ptnumber;k++){
	    if(photon_chiso[position]< chiso && *biggest_pt<highpt[k] && *biggest_pt>lowpt[k]) 
		    ...
		    if(photon_sieie[position]< sieie && photon_isprompt[position]!=1  && isprompt!=1
                       /*&& (tta==false && isprompt!=1) && (za==false && isprompt!=1)*/ ){
			    FakeNumber[k]+=scalef;}
		    ...
		    if(photon_isprompt[position]==1 && (za== true || tta == true)&& isprompt==1 ) 
