...
void cmsLumi(bool channel) 
{
        ...
        latex.DrawLatex(0.76,0.92,Form("59.97 fb^{-1} (%d TeV)", (beamcomenergytev)));
}
...
TH1D* unroll(TH2D* th2_in,Double_t* xbin, Double_t* ybin,  Int_t xbins_in, Int_t ybins_in,char *hname)//,char* htitle)
{
    ...
    TString tag = "_1";
    TString name = hname + tag;
    TH1D* h1= new TH1D(name, "", nbins, 0, nbins);
	...
	for(Int_t iy=1; iy<=nbinsy; iy++){
		for(Int_t ix=1; ix<=nbinsx; ix++){
			...
			Double_t y_temp = 0.5*(ybin[iy-1]+ybin[iy]);////取一个bin两个端点的均值,y direction
               ...
               cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<59.97*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
		}
    }
//        for(Int_t i=0;i<6;i++){
...
//	const char *name[9]={"Mjj 500~800","Mjj 800~1200","Mjj 1200~2000","Mjj 500~800","Mjj 800~1200","Mjj 1200~2000","Mjj 500~800","Mjj 800~1200","Mjj 1200~2000"};
    for(Int_t i=0;i<num;i++){
        ...
        t_ZA[i]= unroll(th2_ZA[i], mjj_bins, detajj_bins, 1,1,Form("hist_%d",i+1));//Form("%d central scale pdf variable",i+1));
        ofstream file2(Form("./scale-txt/content-hist_no%d",i+1));
        ...
        t_ZA[i]->Draw("HIST");
        if(i==0) ll[i]->AddEntry(t_ZA[i],Form("%d central scale variation",i+1));
        else if(i==5)    ll[i]->AddEntry(t_ZA[i],Form("%d scale variation",i+2));
        else if(i==6)    ll[i]->AddEntry(t_ZA[i],Form("%d scale variation",i+3));
        else ll[i]->AddEntry(t_ZA[i],Form("%d scale variation",i+1));
        ll[i]->Draw();
