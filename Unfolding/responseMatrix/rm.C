...
//   RooUnfoldResponse K(5,ptBins[0],ptBins[5],5,ptBins[0],ptBins[5]);
   ...
   TH1D* hist2= new TH1D("hist",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f  reco && gen;leding p_{T}^{reco};yields",ptBins[0],ptBins[5]),5,ptBins);
...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(name.Contains("EWK") ){ 
		      ...
		      hist2->Fill(ptlep1,scalef*lumi);
                      h3->Fill(ptlep1,genlep1pt,scalef*lumi);
	      }
              if(Reco&& !Gen) {
                      i4++;
