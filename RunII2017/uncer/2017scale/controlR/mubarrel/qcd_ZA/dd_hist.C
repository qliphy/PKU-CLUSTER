...
using namespace std;
vector<Double_t> vector_pweight;
vector<int> vector_pos;
TH2D*th2[num];
        ...
        p++;
         ...
         for(Int_t k=0;k<num;k++){
             cout<<"pweight ["<<k<<"] = "<<pweight[k]<<endl;
                 vector_pweight.push_back(pweight[k]);
          }
         std::sort (vector_pweight.begin(), vector_pweight.end(), myobject);
         for(Int_t k=0;k<num;k++){
//             cout<<"pweight ["<<k<<"] = "<<vector_pweight[k]<<endl;
//             cout<<"actualWeight ["<<k<<"] = "<<actualWeight[k]<<endl;
          }
         for (std::vector<double>::iterator it=vector_pweight.begin(); it!=vector_pweight.end(); ++it){
//             int position = distance( begin(vector_pweight), it);
//             std::cout << *it<<endl;
             for(int kk=0;kk<num;kk++ ){
                 if(pweight[kk] - *it ==0){
                     cout<<kk<<" "<<pweight[kk]<<" "<<*it<<endl;
                 }
             }
         }
