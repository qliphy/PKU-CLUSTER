...
TH1D* unroll(TH2D* th2_in,Double_t* xbin, Double_t* ybin,  Int_t xbins_in, Int_t ybins_in,char *hname)//,char* htitle)
{
	...
	TString tag = "_1";
	TH1D* h1= new TH1D(hname+tag, "", nbins, 0, nbins);
	...
	for(Int_t iy=1; iy<=nbinsy; iy++){
		for(Int_t ix=1; ix<=nbinsx; ix++){
			...
			cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<35.86*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
		}
	}
        for(Int_t i=0;i<6;i++){
...
//     t_ZA[0]->GetYaxis()->SetRangeUser(-2,600);
     t_ZA[0]->Draw("HIST");
