...
//   nentries = 1000;
      ...
      for(Int_t k=0;k<ptnumber;k++){
         if(name.Contains("outZA")==1){
            if(photon_drla[position]>1 && photon_drla[position]<10 && 
               ...
               massVlep>80 && massVlep<100&& (Mva>101 || Mva<81) ){
                  //newtree->Fill();
	          if(photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k])
                    {h1[k]->Fill(photon_chiso[position],actualWeight);f++;}
	    }//fake ISR
	 }
         if(name.Contains("outZJets")==1){
            if(photon_drla[position]>0.4 && photon_drla2[position]>0.4&& 
               ...
               massVlep<80 &&
               Mva>81 && Mva<101 &&
	       photon_pt[position]>25/* &&
	       photon_pt[position]>pttruelow[k] && photon_pt[position]<pttruehigh[k]*/)
	    { 
		      //newtree->Fill();
		      h2[k]->Fill(photon_chiso[position],actualWeight);t++;
	    }//mc true FSR 
	 }
         if(name.Contains("outDMuon")==1){
	       ...
	       photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k])
		      {h3[k]->Fill(photon_chiso[position]);df++;}//data fake ISR

            if(photon_drla[position]>0.4 && photon_drla2[position]>0.4 &&
                   TMath::Min(photon_drla[position],photon_drla2[position])<1&&
		   massVlep<80 &&
		   Mva>81 && Mva<101 &&photon_pt[position]>25 /*&&
		   photon_pt[position]>pttruelow[k] && photon_pt[position]<pttruehigh[k]*/)
	               {h4[k]->Fill(photon_chiso[position]);dt++;}//data true FSR

	     if(photon_drla[position]>0.4 && photon_drla2[position]>0.4 &&
	        TMath::Min(photon_drla[position],photon_drla2[position])<1&&
                photon_chiso[position]<0.441 && massVlep<80 &&
		Mva>81 && Mva<101 &&
	        photon_pt[position]>lowpt[k] && photon_pt[position]<highpt[k])
	               {hsieie[k]->Fill(photon_chiso[position]);}//data true photon*/
	 }
      }
         vector_pt.clear();
   }
  cout<<"after photon cut:"<<p<<endl;
  ...
  if(name.Contains("outDMuon")==1)  cout<<"data true "<<dt<<endl;
   ...
   if(name.Contains("outZA")){
       f1 = new TFile("./mcFake_template_chiso.root","recreate");
       f1->cd();
   ...
   if(name.Contains("outDMuon")){
       f3 = new TFile("./dataFake_template_chiso.root","recreate");
       f4 = new TFile("./dataTrue_template_chiso.root","recreate");
       for(Int_t i=0;i<ptnumber;i++){f3->cd();h3[i]->Write();}
       for(Int_t i=0;i<ptnumber;i++){f4->cd();hsieie[i]->Write();h4[i]->Write();}
       file2++;
       cout<<"number of "<<name<<" file:"<<file2<<endl;
     }
   if(name.Contains("outZJets")){
       f2 = new TFile("./mcTrue_template_chiso.root","recreate");
       f2->cd();
...
void MakeTemplate::histo(){
  ...
  for(Int_t i=0;i<ptnumber;i++){
      h2[i]=new TH1D(Form("h2_pt%0.f_%0.f",lowpt[i],highpt[i]),"true template",bin,0,xhigh);
      h1[i]=new TH1D(Form("h1_pt%0.f_%0.f",lowpt[i],highpt[i]),"fake template",bin,0,xhigh);
      h3[i]=new TH1D(Form("h3_pt%0.f_%0.f",lowpt[i],highpt[i]),"data fake template",bin,0,xhigh);
      h4[i]=new TH1D(Form("h4_pt%0.f_%0.f",lowpt[i],highpt[i]),"data true template",bin,0,xhigh);
      hsieie[i]=new TH1D(Form("hsieie_pt%0.f_%0.f",lowpt[i],highpt[i]),"data true photon",bin,0,xhigh);
  }
}
