...
//    cout<<"bin "<<j+1<<"; uncertainty["<<j<<"] = "<<uncertainty_other[j]<<endl;
     cout<<j+1<<"\t"<<fixed<<setprecision(4)<<1-(central_value[j]-smallest[j])/central_value[j]<<" ";
     cout<<" "<<1+(biggest[j]-central_value[j])/central_value[j]<<" "<<biggest[j]-central_value[j]<<endl;
