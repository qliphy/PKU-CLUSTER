...
void hist_2d_mc(std::string filename){
	...
	t->SetBranchAddress("Mjj_JEC_up", &Mjj_up);
	t->SetBranchAddress("Mjj_JEC_down", &Mjj_down);
...
//	TH2D* th2 = new TH2D("th2","th2",4, mjj_bins, 4, mlla_bins);
	...
	for(Int_t i=0; i<t->GetEntries();i++)
	{
		t->GetEntry(i);
                ...
                if(10000*actualWeight<1)
		cout<<i<<"; actualWeight = "<<actualWeight<<"; scalef = "<<scalef<<"; pileupWeight = "<<pileupWeight<<"; lumiWeight = "<<lumiWeight<<endl;
		...
		if(Mjj>=2000 && deltaeta<6.5)  
			th2[0]->Fill(1999, deltaeta, actualWeight);
		if(Mjj_up>=2000 && deltaeta<6.5)  
			th2[1]->Fill(1999, deltaeta, actualWeight);
		if(Mjj_down>=2000 && deltaeta<6.5)  
			th2[2]->Fill(1999, deltaeta, actualWeight);

		if(Mjj<2000 && deltaeta>=6.5)  
			th2[0]->Fill(Mjj_new, 6.1, actualWeight);
		if(Mjj_up<2000 && deltaeta>=6.5)  
			th2[1]->Fill(Mjj_up, 6.1, actualWeight);
		if(Mjj_down<2000 && deltaeta>=6.5)  
			th2[2]->Fill(Mjj_down, 6.1, actualWeight);

		if(Mjj>=2000 && deltaeta>=6.5) 
			th2[0]->Fill(1999, 6.1, actualWeight);
		if(Mjj_up>=2000 && deltaeta>=6.5) 
			th2[1]->Fill(1999, 6.1, actualWeight);
		if(Mjj_down>=2000 && deltaeta>=6.5) 
			th2[2]->Fill(1999, 6.1, actualWeight);
	}
