...
#define num 8
using namespace std;
Double_t lowpt[ptnumber] ={20,25,30,40,60};//j
Double_t highpt[ptnumber]={25,30,40,60,400};

 Double_t lpt[num]= {20,25,30,40,50,60,80,120};//i
 Double_t hpt[num]={25,30,40,50,60,80,120,400};
...
TString b="nosieiecut_muendcap";
TString dir4 = "./txt/";
ofstream file1(dir4 + b+"_closure_FakeUncer.txt");
TString dir = "/home/pku/anying/cms/RunII2018/MakeTemplate/pesudo/muendcap/txt/";
TString dir1 = "/home/pku/anying/cms/RunII2018/MakeTemplate/pesudo/muendcap/roofit/txt/";
void calculate(Double_t lowpt,Double_t highpt,Double_t lowchiso,Double_t highchiso,int i){
    ...
    if(!f3.is_open()){cout<<"can not open the mfakenumber_pt file!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;}
    ...
    if(!f4.is_open()){cout<<"can not open the FakeNumber_pt file!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;}
    f4>>fitFake_number>>fitFake_error>>fit_fakerate>>fit_fakerateErr;
    
    cout<<fixed<<setprecision(0)<<ptlow<<"~"<<fixed<<setprecision(0)<<pthigh<<"\t"<<lpt[i]<<"~"<<fixed<<setprecision(0)<<hpt[i]<<"\t"<<chisolow<<"~"<<chisohigh<<"\t"<<fixed<<setprecision(3)<<FakeNumber<<"\t"<<fitFake_number<<"\t"<<fabs(FakeNumber-fitFake_number)/FakeNumber<<endl;
//    cout<<fixed<<setprecision(0)<<lpt[i]<<"~"<<fixed<<setprecision(3)<<hpt[i]<<"\t"<<fabs(FakeNumber-fitFake_number)/FakeNumber<<endl;
 ...
 void run_calculate(Int_t a,Int_t b){
	...
	if(lowchiso[j]==4) low = lowchiso[j];
        if(highchiso[j]==9) high = highchiso[j];
     }
...
//	  cout<<j<<endl;
	  if(lowpt[j]==lpt[i] && highpt[j]==hpt[i]) 
		  run_calculate(j,i);
	  else if(lowpt[j]==lpt[i] && highpt[j]>hpt[i]) 
		 { run_calculate(j,i);
			 j = j-1;
		 }
	  else if(lowpt[j]<lpt[i] && highpt[j]==hpt[i]) 
