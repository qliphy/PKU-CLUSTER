...
void xx::Loop()
{
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		if(m_dataset=="outDMuon.root"){ scalef=1.0; run_period=1;}
		...
		if(m_dataset=="outZA.root"){ scalef=1000.*124.1/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
		...
		if(m_dataset=="outSTbartW.root"){ scalef=1000.*35.85/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
		...
		if(m_dataset=="outSTbart.root"){ scalef=1000.*80.95/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
