...
jer=[1.019,1.003,1.010,1.08,1.016,1.011,1.017,1.017]
jes=[1.043,1.052,1.070,1.083,1.014,1.061,1.091,1.091]
...
fakephoton=[1.12,1.109,1.27,1.08,1.095,1.084,1.086,1.078];
QCDZA_scale_extra_down = [0.977,0.946,0.919,0.998,1.016,1.029,1.019,1.037];
QCDZA_scale_extra_up = [1.028,1.063,1.10,0.998,0.987,0.974,0.973,0.960];
for i in range(1,9):
   f = open('%s_%s_bin_%d.txt'%('muon17', 'endcap', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on muon17_endcap\n'%('muon17','endcap'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on muon17_endcap\n'%('muon17','endcap',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('muon17','endcap',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('muon17','endcap',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('muon17','endcap',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('muon17','endcap',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('muon17','endcap',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('muon17','endcap',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('muon17','endcap',i,WA_binerror))
