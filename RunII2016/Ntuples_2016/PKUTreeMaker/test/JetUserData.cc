...
//---------------------------- jet only JEC uncertainty
//		RawjetP4--->JEC corr(1+up)--->JER--->smeared jet(up)
//		RawjetP4--->JEC corr(1)--->JER--->smeared jet(center)
//		RawjetP4--->JEC corr(1+down)--->JER--->smeared jet(down)
//---------------------------- jet only JEC uncertainty

		jetParam.setJetPt(jetCorrFactor*(1+jecUncertainty_l1_up)*rawJetP4.pt()).setJetEta(jet.eta()).setRho(*rho);// resolution depend on pt and eta, SF depend on eta and rho, so the parameter should be initialized with three parameters
                float PtResolution_JEC = resolution.getResolution(jetParam);
		float JERSFUp_JEC      = res_sf.getScaleFactor(jetParam, Variation::UP);
                float JERSFDown_JEC    = res_sf.getScaleFactor(jetParam, Variation::DOWN);

                reco::Candidate::LorentzVector smearedP4_JECup =jetCorrFactor*(1+jecUncertainty_up)*rawJetP4;
                reco::Candidate::LorentzVector smearedP4_JECdown =jetCorrFactor*(1+jecUncertainty_down)*rawJetP4;

                smearedP4_JECup *= get_JER_corr( JERSFUp_JEC, isMC, jet, coneSize_, PtResolution_JEC, jetCorrFactor*(1+jecUncertainty_up));
                smearedP4_JECdown *= get_JER_corr( JERSFDown_JEC, isMC, jet, coneSize_, PtResolution_JEC, jetCorrFactor*(1+jecUncertainty_down));
//---------------------------- jet only JEC uncertainty
...
//---------------------------- jet only JER uncertainty
////              RawjetP4--->JEC corr--->JER up--->smeared jet(up)
////              RawjetP4--->JEC corr--->JER center--->smeared jet(center)
////              RawjetP4--->JEC corr--->JER down--->smeared jet(down)
////---------------------------- jet only JER uncertainty
		...
		reco::Candidate::LorentzVector smearedP4 =jetCorrFactor*rawJetP4;
                reco::Candidate::LorentzVector smearedP4_JERup =jetCorrFactor*rawJetP4;
                reco::Candidate::LorentzVector smearedP4_JERdown =jetCorrFactor*rawJetP4;

		smearedP4 *= get_JER_corr( JERSF, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
		smearedP4_JERup *= get_JER_corr( JERSFUp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
		smearedP4_JERdown *= get_JER_corr( JERSFDown, isMC, jet, coneSize_, PtResolution, jetCorrFactor);

		corrEx_MET_JER -= (smearedP4.px() - smearedP4_raw.px());
		corrEx_MET_JER_up -= (smearedP4_JERup.px() - smearedP4_up_raw.px());
		corrEx_MET_JER_down -= (smearedP4_JERdown.px() - smearedP4_down_raw.px());
		corrEy_MET_JER -= (smearedP4.py() - smearedP4_raw.py());
		corrEy_MET_JER_up -= (smearedP4_JERup.py() - smearedP4_up_raw.py());
		corrEy_MET_JER_down -= (smearedP4_JERdown.py() - smearedP4_down_raw.py());
		corrSumEt_MET_JER += (smearedP4.Et() - smearedP4_raw.Et());
		corrSumEt_MET_JER_up += (smearedP4_JERup.Et() - smearedP4_up_raw.Et());
		corrSumEt_MET_JER_down += (smearedP4_JERdown.Et() - smearedP4_down_raw.Et());
		...
		jet.addUserFloat("SmearedPt_JERup",    smearedP4_JERup.pt());
                jet.addUserFloat("SmearedE_JERup",     smearedP4_JERup.energy());
		jet.addUserFloat("SmearedPt_JERdown",    smearedP4_JERdown.pt());
                jet.addUserFloat("SmearedE_JERdown",     smearedP4_JERdown.energy());
		jet.addUserFloat("SmearedPt_JECup",    smearedP4_JECup.pt());
                jet.addUserFloat("SmearedE_JECup",     smearedP4_JECup.energy());
                jet.addUserFloat("SmearedPt_JECdown",    smearedP4_JECdown.pt());
                jet.addUserFloat("SmearedE_JECdown",     smearedP4_JECdown.energy());

// ---------------- only for MET JEC uncertainty-----------------
