...
//		EffectiveAreas effAreaPhotons_;
		...
		std::vector<std::string> muPaths1_, muPaths2_, muPaths3_, muPaths4_, muPaths5_;
		std::vector<std::string> elPaths1, elPaths2, elPaths3, elPaths4, elPaths5;
		std::vector<std::string> muPaths1, muPaths2, muPaths3, muPaths4,muPaths5;
		int  HLT_Ele1;
		int  HLT_Ele2;
		int  HLT_Ele3;
		int  HLT_Ele4;
		...
		edm::EDGetTokenT<std::vector<pat::Jet>> ak4jetsSrc_;
	...
	// ,effAreaPhotons_((iConfig.getParameter<edm::FileInPath>("effAreaPhoFile")).fullPath() )
{
	...
	ak4jetsSrc_      = consumes<std::vector<pat::Jet>>(iConfig.getParameter<edm::InputTag>( "ak4jetsSrc") ) ;
...
//	std::cout<<"matchedgenMu2_pt "<<matchedgenMu2_pt<<std::endl;
	...
	for (size_t ip=0; ip<photons->size();ip++)
	{
		...
		double chiso1=std::max(0.0, chIso1 - rhoVal_*EAch(fabs(phosc_eta))); 
		...
		double nhiso1=std::max(0.0, nhIso1 - rhoVal_*EAnh(fabs(phosc_eta))); 
		...
		double phoiso1=std::max(0.0, phIso1 - rhoVal_*EApho(fabs(phosc_eta))); 
