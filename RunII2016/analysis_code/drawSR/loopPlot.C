...
void loopPlot() {
	...
	std::string pathToTrees = "/data/pku/home/anying/cms/CMSSW_8_0_26_patch2/analysis_code/selection_code/";
...
// Setup names of data files for trees.
	...
	std::cout << "set data imformation, we have " << nDATA << "data file"
			<< std::endl;
	std::string dataLabels[nDATA] = { "DMuon" };
	std::vector < std::string > fData;
	for (int ii = 0; ii < nDATA; ii++) {
		fData.push_back(pathToTrees + "SR-out" + dataLabels[ii] + ".root");
	}
// set mc imformation
		const int nMC = 2;
		...
		for (int ii = 0; ii < nMC; ii++) {
			fMC.push_back(pathToTrees +"SR-out"+ mcLabels[ii] + ".root");
		}
...
// set mcsig information
	...
	for (int ii = 0; ii < nMCSig; ii++) {
		fMCSig.push_back(pathToTrees + "SR-out" + mcLabelsSig[ii] + ".root");
	}
