...
ofstream myfile("../muendcap/fraction/result_TFraction.txt");
ofstream file3("../muendcap/fraction/info_fitifraction.txt");
ofstream file("../muendcap/fraction/detail_frac.txt");
void fit(float lowpt, float highpt, float lowchiso, float highchiso){
  ...
  if (status == 0) {                       // check on fit status
	...
	textchiso->Draw();
     c1->SaveAs(Form("../muendcap/fraction/eps/fakefraction_pt%0.f-%0.f_chiso%0.f-%0.f.eps",lowpt,highpt,lowchiso,highchiso));
   }
//fdata->Close();
//ftrue->Close();
}
