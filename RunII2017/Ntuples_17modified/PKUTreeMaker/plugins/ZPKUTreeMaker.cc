...
//		virtual void addTypeICorr_user( edm::Event const & event );//---for MET, Meng
		...
		edm::EDGetTokenT<edm::View<pat::Electron> > fakeelectronToken_ ; 
		...
		edm::EDGetTokenT<edm::View<pat::Muon> > loosetightmuonToken_;
		...
		// for muon rochester correction
		int  lep, nlooseeles,nloosemus, ngoodmus,ngoodeles,nloosetightmus,nfakeeles;
		bool looseMuon,tightMuon,loosetightMuon,mediumEle,fakeEle,looseEle;
		...
		double photon_hoe[6],photon_sieie[6],photon_sieie2[6],photon_chiso[6],photon_nhiso[6],photon_phoiso[6],photon_drla[6],photon_drla2[6],photon_mla[6],photon_mla2[6],photon_mva[6];
                double photon_chiso1[6],photon_nhiso1[6],photon_phoiso1[6];
...
//	t1jetSrc_      = consumes<pat::Jet>(iConfig.getParameter<edm::InputTag>( "t1jetSrc") ) ;
	...
	fakeelectronToken_    = (consumes<edm::View<pat::Electron> > (iConfig.getParameter<edm::InputTag>("fakeelectronSrc"))) ;
	...
	loosetightmuonToken_    = (consumes<edm::View<pat::Muon> > (iConfig.getParameter<edm::InputTag>("loosetightmuonSrc")))              ;
	...
	outTree_->Branch("looseMuon"     ,&looseMuon      ,"looseMuon/O"         );
	outTree_->Branch("loosetightMuon",&loosetightMuon ,"loosetightMuon/O"    );
	outTree_->Branch("tightMuon"     ,&tightMuon      ,"tightMuon/O"         );
	outTree_->Branch("mediumEle"     ,&mediumEle      ,"mediumEle/O"         );
	outTree_->Branch("looseEle"      ,&looseEle       ,"looseEle/O"          );
	outTree_->Branch("fakeEle"       ,&fakeEle        ,"fakeEle/O"           );
	outTree_->Branch("nlooseeles"    ,&nlooseeles     ,"nlooseeles/I"        );
	outTree_->Branch("ngoodeles"     ,&ngoodeles      ,"ngoodeles/I"         );
	outTree_->Branch("nfakeeles"     ,&nfakeeles      ,"nfakeeles/I"         );
	outTree_->Branch("nloosemus"     ,&nloosemus      ,"nloosemus/I"         );
	outTree_->Branch("ngoodmus"      ,&ngoodmus       ,"ngoodmus/I"          );
	outTree_->Branch("nloosetightmus",&nloosetightmus ,"nloosetightmus/I"    );
	...
	outTree_->Branch("photon_phoiso"    ,photon_phoiso    ,"photon_phoiso[6]/D"  );
        outTree_->Branch("photon_chiso1"    ,photon_chiso1    ,"photon_chiso1[6]/D"  );
        outTree_->Branch("photon_nhiso1"    ,photon_nhiso1    ,"photon_nhiso1[6]/D"  );
        outTree_->Branch("photon_phoiso1"   ,photon_phoiso1   ,"photon_phoiso1[6]/D" );
...
//			std::cout<<"e "<<genjet_e[ijj]<<std::endl;//gen jet debug
	...
	edm::Handle<edm::View<pat::Muon>> loosetightmus;
	iEvent.getByToken(loosetightmuonToken_,loosetightmus);
	edm::Handle<edm::View<pat::Electron>> goodeles;
	iEvent.getByToken(goodeleToken_,goodeles);
	edm::Handle<edm::View<pat::Electron>> fakeeles;
	iEvent.getByToken(fakeelectronToken_,fakeeles); 
...
//		addTypeICorr_user(iEvent);
	...
	if(fakeeles->size()>1){fakeEle = true;}
	if(looseeles->size()>1){looseEle = true;}
	// muon station2 retrieve, L1 issue, Meng 2017/3/26
	if(goodmus->size()>1){
                tightMuon=true;
        ...
        if(loosemus->size()>1){looseMuon=true;}
        if(loosetightmus->size()>1){loosetightMuon=true;}
	// Lu
...
//	std::cout<<"matchedgenMu2_pt "<<matchedgenMu2_pt<<std::endl;
	...
	nfakeeles = fakeeles->size();
	nloosetightmus = loosetightmus->size();
	...
	for (size_t ip=0; ip<photons->size();ip++)
	{
		...
		double pho_ieie=(*photons)[ip].full5x5_sigmaIetaIeta();//(*full5x5SigmaIEtaIEtaMap)[pho];
                ...
                double chIso2 = (*photons)[ip].userFloat("phoChargedIsolation");
                //double nhIso1 = (*photons)[ip].neutralHadronIso();// (*phoNeutralHadronIsolationMap)[pho];
                double nhIso2 = (*photons)[ip].userFloat("phoNeutralHadronIsolation");
		//double phIso1 = (*photons)[ip].photonIso();//(*phoPhotonIsolationMap)[pho];
                double phIso2 = (*photons)[ip].userFloat("phoPhotonIsolation");
		double chiso=std::max(0.0, chIso2 - rhoVal_*EAch(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaChHadrons_.getEffectiveArea(fabs(phosc_eta)));
                double chiso1=std::max(0.0, chIso2 - rhoVal_*EAch(fabs(phosc_eta)));
		//            double chiso=std::max((*photons)[ip].chargedHadronIso()-rhoVal_*EAch(fabs((*photons)[ip].eta())),0.0);
		double nhiso=std::max(0.0, nhIso2 - rhoVal_*EAnh(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaNeuHadrons_.getEffectiveArea(fabs(phosc_eta)));
                double nhiso1=std::max(0.0, nhIso2 - rhoVal_*EAnh(fabs(phosc_eta)));
		//            double nhiso=std::max((*photons)[ip].neutralHadronIso()-rhoVal_*EAnh(fabs((*photons)[ip].eta())),0.0);
		double phoiso=std::max(0.0, phIso2 - rhoVal_*EApho(fabs(phosc_eta/*(*photons)[ip].eta()*/))); //effAreaPhotons_.getEffectiveArea(fabs(phosc_eta)));
                double phoiso1=std::max(0.0, phIso2 - rhoVal_*EApho(fabs(phosc_eta)));
		...
		if(ip<6)  {
			...
			photon_phoiso[ip]=phoiso;
                        photon_chiso1[ip]=chiso1;
                        photon_nhiso1[ip]=nhiso1;
                        photon_phoiso1[ip]=phoiso1;
