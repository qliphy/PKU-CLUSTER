...
#define num 9
using namespace std;
Double_t lowpt[ptnumber] ={20,25,30,40,50,60};//j
Double_t highpt[ptnumber]={25,30,40,50,60,400};

 Double_t lpt[num]= {20,25,30,35,40,50,60,80,120};//i
 Double_t hpt[num]={25,30,35,40,50,60,80,120,400};
...
TString b="nosieiecut_elebarrel";
TString dir4 = "./txt/";
ofstream file1(dir4 + b+"_closure_FakeUncer.txt");
TString dir  = "/home/pku/anying/cms/RunII2016/analysis_code/MakeTemplate/ZAJetfakephoton/pesudo/elebarrel/txt/";
TString dir1 = "/home/pku/anying/cms/RunII2016/analysis_code/MakeTemplate/ZAJetfakephoton/pesudo/elebarrel/roofit/txt/";
...
//    cout<<fixed<<setprecision(0)<<ptlow<<"~"<<fixed<<setprecision(0)<<pthigh<<"\t"<<chisolow<<"~"<<chisohigh<<"\t"<<fixed<<setprecision(3)<<FakeNumber<<"\t"<<fitFake_number<<"\t"<<fabs(FakeNumber-fitFake_number)/FakeNumber<<endl;
    cout<<fixed<<setprecision(0)<<lpt[i]<<"~"<<hpt[i]<<"\t"<<chisolow<<"~"<<chisohigh<<"\t"<<fixed<<setprecision(3)<<FakeNumber<<"\t"<<fitFake_number<<"\t"<<fabs(FakeNumber-fitFake_number)/FakeNumber<<endl;
 ...
 void run_calculate(Int_t a,Int_t b){
	...
	if(lowchiso[j]==5) low = lowchiso[j];
