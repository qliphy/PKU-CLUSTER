...
jer=[1.015,1.004,1.009,1.039,1.013,1.009,1.068,1.006]
jes=[1.021,1.034,1.024,1.072,1.095,1.079,1.027,1.044]
pdf_QCD=[1.01,1.01,1.03,1.01,1.01,1.02,1.01,1.01]
pdf_ewk=[1.04,1.04,1.04,1.04,1.04,1.04,1.05,1.04]
scale_QCD=[1.15,1.21,1.18,1.09,1.10,1.10,1.09,1.08]
scale_ewk=[1.04,1.06,1.09,1.03,1.06,1.10,1.05,1.13]
fakephoton=[1.124,1.156,1.081,1.140,1.172,1.141,1.105,1.106];
QCDZA_scale_extra_down = [0.977,0.946,0.919,0.998,1.016,1.029,1.052,1.037];
QCDZA_scale_extra_up = [1.028,1.063,1.10,0.998,0.987,0.974,0.980,0.960];
for i in range(1,9):
   f = open('%s_%s_bin_%d.txt'%('ele17', 'endcap', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on ele17_endcap\n'%('ele17','endcap'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on ele17_endcap\n'%('ele17','endcap',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('ele17','endcap',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('ele17','endcap',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('ele17','endcap',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('ele17','endcap',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('ele17','endcap',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('ele17','endcap',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('ele17','endcap',i,WA_binerror))
