...
// Fri Aug 31 16:04:50 2018 by ROOT version 6.10/09
// from TTree EventTree/Event data (tag V09_04_09_00)
// found on file: test3_mc.root
//////////////////////////////////////////////////////////

#ifndef EGMregression_h
#define EGMregression_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   EGMregression(TTree *tree=0);
   virtual ~EGMregression();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(bool isData);
...
#ifdef EGMregression_cxx
EGMregression::EGMregression(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("test3_mc.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("test3_mc.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("test3_mc.root:/ggNtuplizer");
...
EGMregression::~EGMregression()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t EGMregression::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t EGMregression::LoadTree(Long64_t entry)
{
...
void EGMregression::Init(TTree *tree)
{
...
void EGMregression::Show(Long64_t entry)
{
...
Int_t EGMregression::Cut(Long64_t entry)
{
