...
TH1D* trans_fsr(TTree* treename, TString filename){

	char buffer1[256];
//        sprintf(buffer1, "%s",filename.c_str());
        TTree* t1=treename;
	TH1D*h=new TH1D(filename,buffer1,1000,0,0.07);
	...
	for(Long64_t i=0;i<entries;i++){
		...
		double pt_tmp=-99;
                ...
                for(int j=0;j<6;j++){
                        ...
                        if(photon_pt[j]>15 && photon_hoe[j]<0.0396 && photon_nhiso[j]<(2.725+0.0148*photon_pt[j]+0.000017*photon_pt[j]*photon_pt[j]) && photon_phoiso[j]<(2.571+0.0047*photon_pt[j]) && photon_chiso[j]<0.441 &&tm1>70 && tm1<110/* && lep == 11 && ptlep1 > 25. && ptlep2 > 25. && fabs(etalep1) < 2.5 && fabs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus ==0 && massVlep > 70. && massVlep < 110.*/) {
                                if(i<100) std::cout<<"pass"<<std::endl;
                                if(j==0) {iii=j;pt_tmp=photon_pt[iii];}
                                if(j>0) {if(photon_pt[j]>pt_tmp){ iii=j; pt_tmp=photon_pt[iii];}}
                        }
                }
//		if(iii<0 || !(fabs(photon_eta[iii])>1.556 && fabs(photon_eta[iii])<2.5) ) continue;//endcap
		if( iii<0 || !(fabs(photon_eta[iii])<1.4442) ) continue;
		weight=1.;
		if(filename.Contains("ZJets") ){
			scalef=0.070591*fabs(theWeight)/theWeight;
...
//	TTree*tdata=(TTree*)fdata->Get("DEle16");
	TFile* fdata=TFile::Open("/home/pku/anying/cms/file_in_cms/cutLEP/cutlep-outDEle16.root");
	TTree*tdata=(TTree*)fdata->Get("demo");
//	TFile* fmc=TFile::Open("../../samples/barrel/smallZJets_FX.root");
//	TTree*tmc=(TTree*)fmc->Get("ZJets_FX");
	TFile* fmc=TFile::Open("/home/pku/anying/cms/file_in_cms/cutLEP/cutlep-outZJets_FX.root");
	TTree*tmc=(TTree*)fmc->Get("demo");
