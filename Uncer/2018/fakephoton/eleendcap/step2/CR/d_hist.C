...
//	t->SetBranchAddress("deltaetajj", &deltaetajj);
	...
	Double_t deltaetajj_bins[2]={0, 6.5};
        TH2D* th2[3]; 
	char th2name[num];
	sprintf(th2name,"th2_%d",i);
	th2[i] = new TH2D(th2name,th2name,1, mjj_bins, 1, deltaetajj_bins);
	th2[i]->Sumw2();

	for(Int_t j=0; j<t->GetEntries();j++)
	{
		t->GetEntry(j);
		if( !(lep==11 && ( fabs(photoneta)>1.566 && fabs(photoneta)<2.5 )&& Mjj>150 && Mjj<400) ) continue;
//		if( !(lep==11 && ( fabs(photoneta)<1.4442 )&& Mjj>150 && Mjj<400) ) continue;
//		cout<<"scalef "<<scalef<<endl;
		if(Mjj<400 && deltaetajj<6.5)   th2[i]->Fill(Mjj,deltaetajj,scalef);
		if(Mjj<400 && deltaetajj>=6.5)  th2[i]->Fill(Mjj,6.1,scalef);
...
int d_hist(){
	TH2D* h0 = run("/home/pku/anying/cms/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap.root",0);
	TH2D* hup = run("/home/pku/anying/cms/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap_up.root",1);
	TH2D* hdown = run("/home/pku/anying/cms/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap_down.root",2);
