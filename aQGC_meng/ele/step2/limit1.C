...
#include <vector.h>
	...
	files.push_back("optimal_outDEle");
	files.push_back("optimal_outphotonlikejet_ele_mediumlikephoton");
	...
	{
		TString filename = "../../../for_aQGC/ele_barrelandendcap/"+files.at(i)+".root";
		...
		for(int j=0; j<nentries1; j++)
		{
			...
			bool ipass=photonet>100;

			double lumi=35.866; 

			if(hname=="ZG_optimal_outDEle"||hname=="ZG_optimal_outphotonlikejet_ele_mediumlikephoton") {
				lumi=1.0;
			}
