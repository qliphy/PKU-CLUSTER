...
void open(Int_t i);
const int p=3;//the amount of bin
const int histo_number= num-1;// the number of histo except centeral 
const int number=num;// the number of histo except centeral 
...
void open(Int_t i){
    ...
    for(Int_t j=0;j<p;j++){
       if(i==1){
         ...
         if(central_value[j]==0) central_value[j]=0.1;
       if(i>1){
          file>>nlo_value[i-2][j];
	  if(nlo_value[i-2][j]==0) nlo_value[i-2][j]=0.1;
...
//    cout<<"bin "<<j+1<<"; uncertainty["<<j<<"] = "<<uncertainty_other[j]<<endl;
     cout<<"bin "<<j+1<<"; up uncertainty["<<j<<"] = "<<(biggest[j]-central_value[j])/central_value[j]<<endl;
     cout<<"bin "<<j+1<<"; down uncertainty["<<j<<"] = "<<(central_value[j]-smallest[j])/central_value[j]<<endl;
    f1<<"bin "<<j+1<<"; uncertainty["<<j<<"] = "<<uncertainty_other[j]<<endl;
  }
const char *name[p]={"Mjj 150~400","Mjj 150~400","Mjj 150~400"};
TH1D* h1 =new TH1D("h1","uncertainty",p,0,3);
for(Int_t i=0;i<p;i++){
