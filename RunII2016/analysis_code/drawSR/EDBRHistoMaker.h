...
void EDBRHistoMaker::createAllHistos() {
	...
	hs.setHisto("massVlep",90, 30, 200);
...
void EDBRHistoMaker::Loop(std::string outFileName) {
	...
	double r2=gRandom->Rndm(jentry*2);
		...
		if (drll>0.3 &&l1_weight==1&&lep == 13 &&  ((run_period<7 && HLT_Mu5 > 0)|| (run_period==7 && HLT_Mu2 > 0)) && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 20. && massVlep < 200. && photonet > 25.&& fabs(photoneta) < 1.4442 ) {
			numbe_out++;
			treename->Fill();
		}
		else
		...
		if (isnotwets > 0 || iswjets > 0 || iszjets > 0 || isttjets > 0) {
			...
			(theHistograms["drla2"])->Fill(drla2, actualWeight);
...
//                }
		...
		if (drll>0.3  &&l1_weight==1&&lep == 13 && HLT_Mu2 > 0 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 20. && massVlep < 200. && photonet > 25.&& fabs(photoneta) < 1.4442) {
			numbe_out++;
			treename->Fill();
		...
		actualWeight = actualWeight*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*muon1_track_scale*muon1_track_scale*muon_hlt_scale;
