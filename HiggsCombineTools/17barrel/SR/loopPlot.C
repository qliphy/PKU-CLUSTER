...
void loopPlot() {
	...
	std::string pathToTrees = "/home/pku/anying/cms/RunII2017/aa2/Barrel/output-slimmed-rootfiles/";
...
// Setup names of data files for trees.
	...
	for (int ii = 0; ii < nDATA; ii++) {
		fData.push_back(pathToTrees+"optimal_2017_" + dataLabels[ii] + ".root");
	}
// set mc imformation
		const int nMC = 6;
		std::cout << "set data imformation, we have " << nMC << "mc file"
				<< std::endl;
		std::string mcLabels[nMC] = { "ST", "TTA", "VV", "WA","plj_barrel", "ZA" };
		double kFactorsMC_array[nMC] = { 1, 1, 1, 1, 1, 1};
		std::vector< std::string > fMC;
		for (int ii = 0; ii < nMC; ii++) {
			fMC.push_back(pathToTrees + "optimal_2017_" + mcLabels[ii] + ".root");
		}
...
// set mcsig information
	...
	for (int ii = 0; ii < nMCSig; ii++) {
		fMCSig.push_back(pathToTrees+"optimal_2017_" + mcLabelsSig[ii] + ".root");
	}
