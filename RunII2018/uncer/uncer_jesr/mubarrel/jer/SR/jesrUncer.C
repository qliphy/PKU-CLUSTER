...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   mjj_JEC_down = new TH1D("h_down","mjj_JER_down",9,0,9);
   mjj_JEC_up = new TH1D("h_up","mjj_JER_up",9,0,9);
   for(int i=0;i<3;i++){
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   if(  !( drla>0.7 && drla>0.7 && lep == 13 && (HLT_Mu1>0 || HLT_Mu2 > 0) && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && photonet > 20.&& fabs(photoneta) < 1.4442) )
		   continue;
...
//	   cout<<"scalef = "<<scalef<<"; pileupWeight = "<<pileupWeight<<"; actualWeight = "<<actualWeight<<endl;
	   for(int i=0;i<3;i++){
		   if(JET_JER[i] && Mjj_JER[i]<2000 && deltaeta_JER[i]<6.5){ 
